...
const shared=()=>getCache({namespace:'kis-server'});
const tokenKey=()=>createHash('sha256').update(process.env.KIS_APP_KEY+'|'+process.env.KIS_APP_SECRET).digest('hex');
async function cacheGet(key){if(!process.env.VERCEL)return;try{return await shared().get(key)}catch{}}
async function cacheSet(key,value,ttl){if(!process.env.VERCEL)return;try{await shared().set(key,value,{ttl})}catch{}}
...
async function accessToken() {
  ...
  pending = (async () => {
    const saved=await cacheGet('token:'+tokenKey());
    if(saved?.token&&saved.expires>Date.now()){token=saved.token;expires=saved.expires;return token}
    ...
    if (!r.ok || !body.access_token) {const code=/^[A-Z0-9_]{1,24}$/.test(body.error_code||'')?body.error_code:'AUTH_FAILED';throw Error(code)}
    token = body.access_token;
    expires = Date.now() + Math.max(0,(Number(body.expires_in)||86400)-300)*1000;
    await cacheSet('token:'+tokenKey(),{token,expires},Math.max(1,Math.floor((expires-Date.now())/1000)));
    return token;
...
export async function enrichKis(data) {
  ...
  let bearer;try {bearer=await accessToken()} catch(e) {return {...data,kis:{configured:true,connected:false,count:0,message:'KIS 인증 실패 또는 재발급 대기 · 보조 시세 사용',code:/^[A-Z0-9_]{1,24}$/.test(e.message)?e.message:'AUTH_WAIT'}}}
  let count=0,flows=0;
  const today=date(Date.now());
  if(calendarCache?.date!==today)calendarCache=await cacheGet('calendar:'+today);
  ...
  if(typeof calendarCache?.open==='boolean')await cacheSet('calendar:'+today,calendarCache,86400);
